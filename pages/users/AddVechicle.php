<?php
	...
	if(isset($_POST['submit'])){
		...
		/*  IMAGE UPLOAD    */
			...
			/* bill book upload*/
			$newfilenum = 1;
			foreach($_FILES["billBookFileToUpload"]["name"] as $key=>$val){
			$target_dir = "../images/BillBook/".$nextVal."/";
			$target_file = $target_dir . basename($_FILES["billBookFileToUpload"]["name"][$key]);
			$uploadOk = 1;
			$imageFileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));
			$check = getimagesize($_FILES["billBookFileToUpload"]["tmp_name"][$key]);
			if($check !== false) {
		...
		// if everything is ok, try to upload file
			...
			$temp = explode(".", $_FILES["billBookFileToUpload"]["tmp_name"][$key]);
		$newfilename = $newfilenum.".jpg";
		$newfilenum = $newfilenum + 1;
		if(move_uploaded_file($_FILES["billBookFileToUpload"]["tmp_name"][$key], $target_dir . $newfilename)){
